...
public class Graf {
    ...
    public void SzelessegiBejar(int kezdopont) {
        List<Integer> bejart = new LinkedList<>();
        List<Integer> kovetkezok = new LinkedList<>();

        kovetkezok.add(kezdopont);
        bejart.add(kezdopont);

        while (kovetkezok.size() > 0) {
            var k = kovetkezok.get(0);
    ...
    public void MelysegiBejar1(int kezdopont){
        List<Integer> bejart = new LinkedList<>();
        List<Integer> kovetkezok = new LinkedList<>();
        ...
        while (kovetkezok.size()>0){
            var k=kovetkezok.get(kovetkezok.size());
            kovetkezok.remove(kovetkezok.size());
    ...
    public boolean Osszefuggo(){
        ...
        if(bejart.size()==this.csucsokSzama) {
            return true;
        }
        else {
            return false;
    ...
    public HashMap Mohoszinezes(){
        ...
        var maxszín = this.csucsokSzama;

        for (int i=0; i<this.csucsokSzama-1;i++){
            List<Integer> valaszthatoSzinek=new LinkedList<>();
